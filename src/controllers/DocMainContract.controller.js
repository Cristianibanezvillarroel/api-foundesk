const DocMainContract = require('../models/DocMainContract.model')

/**
 * Obtener todas las versiones del contrato principal
 */
const getAllVersions = async (req, res) => {
  try {
    const versions = await DocMainContract.find()
...
/**
 * Obtener la versión activa del contrato principal
 */
const getActiveVersion = async (req, res) => {
  try {
    const activeVersion = await DocMainContract.findOne({ status: 'active' })
      .populate('createdBy', 'name lastname email')
      .populate('updatedBy', 'name lastname email')
      .sort({ versionDate: -1 })
    ...
    if (!activeVersion) {
      return res.status(404).json({
        message: 'No hay versión activa del contrato',
        detail: null
      })
    }
...
const getVersionById = async (req, res) => {
  try {
    ...
    const version = await DocMainContract.findById(id)
      .populate('createdBy', 'name lastname email')
      .populate('updatedBy', 'name lastname email')
...
/**
 * Crear una nueva versión del contrato principal
 */
const createVersion = async (req, res) => {
  try {
    ...
    const existingVersion = await DocMainContract.findOne({ version })
    if (existingVersion) {
    ...
    const newVersion = new DocMainContract({
      version,
      description,
      foundeskData: foundeskData || {},
      clauses: clauses || [],
      notes,
      status: 'draft',
...
const updateVersion = async (req, res) => {
  try {
    ...
    console.log('📝 UPDATE VERSION - Clauses received:', {
      clausesType: Array.isArray(clauses) ? 'array' : typeof clauses,
      clausesLength: clauses?.length,
      hasClauses: !!clauses
    })

    const existingVersion = await DocMainContract.findById(id)
    if (!existingVersion) {
      return res.status(404).json({
        message: 'Versión no encontrada',
        detail: null
    ...
    // No permitir editar versiones activas o superseded (opcional, según lógica de negocio)
    ...
    if (foundeskData) existingVersion.foundeskData = { ...existingVersion.foundeskData, ...foundeskData }
...
const deleteVersion = async (req, res) => {
  try {
    ...
    await DocMainContract.findByIdAndDelete(id)
